...
public class Enemy extends Entity implements Poolable {
    ...
    private Image[] sprites = new Image[4]; // Mảng chứa 4 frames ảnh
    private int currentFrame = 0;            // Frame hiện tại (0, 1, 2, 3)
    private double frameTimer = 0;           // Biến tích lũy thời gian
    private static final double FRAME_DURATION = 0.15; // Thời gian đổi frame (0.15s)
    ...
    public void initialize(EnemyType type, MapModel mapModel) {
        ...
        this.sprites = loadSprites(type); // Nạp mảng 4 frames animation
        this.currentFrame = 0;
        this.frameTimer = 0;
    ...
    public void update(double deltaTime) {
        ...
        // cập nhập animation frame
        frameTimer += deltaTime;
        if (frameTimer >= FRAME_DURATION) {
            frameTimer -= FRAME_DURATION;
            currentFrame = (currentFrame + 1) % 4; // Vòng lặp từ 0 -> 1 -> 2 -> 3 -> 0
        }
    ...
    public void render(GraphicsContext gc) {
        ...
        // Lấy frame ảnh hiện tại
        Image currentImg = (sprites != null && currentFrame < sprites.length) ? sprites[currentFrame] : null;
    ...
    private Image[] loadSprites(EnemyType type) {
        if (type == null) {
            return null;
        }
        ...
        for (int i = 1; i <= 4; i++) {
            String path = prefix + i + ".png"; // Ví dụ: /assets/quai1.1.png
